...
with st.sidebar:
    selected = option_menu(None,
                          ...
                          icons=['house-fill','bi bi-meta','thermometer-half','bi bi-heart','bar-chart'],
                          default_index=0)
...
if selected=="Heart Disease":
    ...
    def main():
        ...
        label_2=f"Fasting Blood Sugar: [{link_text}]({link_url_2})"
        fs = st.slider(label_2,80,300)
        if fs>=120:
            fbs =1
        if fs<120:
            fbs=0
...
if selected == "Diabetes(Women)":
    ...
    def main():
        ...
        DiabetesPedigreeFunction = st.slider("DiabetesPedigreeFunction",0.00,1.00)
        ...
        if st.button("Predict"):
            ...
            print(result)
            if result == 1:
                st.error("Data is affected by Diabetics")
            else:
                st.success("Data is not affected by Diabetics")
    if __name__=='__main__':
	    main()
